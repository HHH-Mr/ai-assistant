...
def get_retriever():
    ...
    # 向量数据库持久化路径
    persist_directory = '/root/project/data_base/vector_db/chroma'
    # 加载数据库
    vectordb = Chroma(
...
def combine_docs(docs):
    ...
    if not docs:
        print("[WARN] 没有检索到任何文档！")
    return "\n\n".join(doc.page_content for doc in docs)

from zhipuai_llm import ZhipuaiLLM
def get_qa_history_chain():
    retriever = get_retriever()
    llm = ZhipuaiLLM(model_name="glm-4.7", temperature=0, api_key=api_key)
    ...
    qa_chain = (
        RunnablePassthrough.assign(
            context=lambda x: combine_docs(x["context"])   # x 是完整字典，取 "context" 键
        )
        | qa_prompt
        | llm
        | StrOutputParser()
